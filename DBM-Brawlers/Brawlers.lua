...
function mod:CHAT_MSG_MONSTER_YELL(msg, npc, _, _, target)
	...
	elseif msg:find(L.Rank10, 1, true) then
		currentFighter = target
		currentRank = 10
	elseif currentFighter and target == currentFighter and not msg:find(L.BizmoIgnored) and not msg == L.BizmoIgnored then--He's targeting current fighter but it's not a match begin yell, the only other time this happens is on match end.
