...
do
	...
	local function handleEvent(self, event, ...)
		...
		for i, v in ipairs(registeredEvents[event]) do
			...
			if handler and (not isUnitEvent or not modEvents or modEvents[event .. ...]) and (not zones or zones[LastInstanceMapID]) and not (not v.isTrashMod or v.isTrashMod and #inCombat > 0) then
				handler(v, ...)
			end
		end
	end
