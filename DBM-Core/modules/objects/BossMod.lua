	...
	function bossModPrototype:CheckDispelFilter(dispelType)
		...
		if dispelType then
			...
			for spellID, _ in pairs(typeCheck[dispelType]) do
				if typeCheck[dispelType][spellID] and DBM:IsSpellKnown(spellID) and (DBM:GetSpellCooldown(spellID)) == 0 then--Spell is known and not on cooldown
					lastCheck = GetTime()
...
do
	...
	function bossModPrototype:CheckCCFilter(ccType)
		...
		for spellID, _ in pairs(typeCheck[ccType]) do
			if typeCheck[ccType][spellID] and DBM:IsSpellKnown(spellID) and (DBM:GetSpellCooldown(spellID)) == 0 then--Spell is known and not on cooldown
				lastCheck = GetTime()
				lastReturn = true
				return lastReturn
			end
