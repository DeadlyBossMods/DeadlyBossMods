...
L.NEWS_UPDATE						= "|h|c11ff1111Новости|r|h: DBM был обновлен с изменениями в структуре модов, поэтому Классическая и Актуальная версии теперь могут использовать унифицированные (одинаковые) модули. Это означает, что рейдовые модули Vanilla (включая Сезон открытий), TBC, Wrath и Cata теперь необходимо загружать отдельно, используя те же пакеты, что и Актуальные. Подробнее об этом |Haddon:DBM:news|h|cff3588ff[нажмите здесь]|r|h"
...
L.BIG_WIGS							= "BigWigs"
L.WEAKAURA_KEY						= " (|cff308530ID заклинания:|r %s)"
