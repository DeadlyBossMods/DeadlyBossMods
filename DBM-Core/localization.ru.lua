...
L.HARDCODED_FALLBACK				= L.DBM .. " обнаружил неожиданную проблему в модуле с жёстко заданной логикой. " .. L.DBM .. " будет использовать таймеры и предупреждения Blizzard для этого боя."
...
L.SLASHCMD_HELP2						= {
	...
	"/dbm version2: Выполняет проверку версии босс-мода; шепчет игрокам, у которых устаревшая версия (псевдоним: ver2).",
...
L.AUTO_ANNOUNCE_OPTIONS.sooncount		= "Заранее предупреждать о $spell:%s (со счётчиком)"
...
L.AUTO_SPEC_WARN_TEXTS.blizztarget		= "%s (%%s) на >%%s< "
L.AUTO_SPEC_WARN_TEXTS.blizzyou			= "%s (%%s) на Вас"
...
L.AUTO_SPEC_WARN_OPTIONS.blizztarget	= "Спецпредупреждение (со счётчиком), когда на ком-то $spell:%s"
L.AUTO_SPEC_WARN_OPTIONS.blizzyou		= "Спецпредупреждение (со счётчиком), когда на Вас $spell:%s"
...
L.AUTO_TIMER_TEXTS.addscount		= "Адды (%%s)"
...
L.AUTO_TIMER_OPTIONS.addscount		= "Отсчёт времени (со счетчиком) до прибытия аддов"
...
L.MN_TEXT_TIMERS_HEADER	= "Хотите использовать текстовые таймеры?\n(Используются данные полос DBM, даже если полосы не отображаются)"
