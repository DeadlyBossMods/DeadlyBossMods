...
local yellParanoiaRepeater					= mod:NewPosYell(309980, DBM_CORE_Translations.AUTO_YELL_ANNOUNCE_TEXT.shortyell)
...
local timerMindwrackCD						= mod:NewCDTimer(5.6, 316711, nil, "Tank", 2, 5, nil, DBM_CORE_Translations.TANK_ICON)--4.9-8.6
local timerCreepingAnguishCD				= mod:NewNextTimer(28.2, 310184, nil, nil, 2, 5, nil, DBM_CORE_Translations.TANK_ICON)
local timerSynampticShock					= mod:NewBuffActiveTimer(30, 313184, nil, nil, nil, 5, nil, DBM_CORE_Translations.DAMAGE_ICON)--, nil, 1, 4
----Mind's Eye
...
local timerEternalTormentCD					= mod:NewNextCountTimer(56.1, 318449, 311383, nil, nil, 2, nil, DBM_CORE_Translations.HEALER_ICON)--"Torment" short name
----Basher Tentacle
mod:AddTimerLine(DBM:EJ_GetSectionInfo(21286))
local timerBasherTentacleCD					= mod:NewNextCountTimer(60, "ej21286", nil, nil, nil, 1, "319441", DBM_CORE_Translations.DAMAGE_ICON)
local timerVoidLashCD						= mod:NewCDTimer(22.9, 309698, nil, false, 2, 5, nil, DBM_CORE_Translations.TANK_ICON)
----Through the MindGate
...
local timerStupefyingGlareCD				= mod:NewNextCountTimer(22.9, 317874, 239918, nil, 2, 3, nil, DBM_CORE_Translations.DEADLY_ICON, nil, 1, 5)
----Thought Harvester
...
local timerEventHorizonCD					= mod:NewNextCountTimer(22.9, 318196, nil, nil, nil, 5, nil, DBM_CORE_Translations.TANK_ICON)--, nil, 2, 4
...
mod:AddMiscLine(DBM_CORE_Translations.OPTION_CATEGORY_DROPDOWNS)
...
local function stupefyingGlareLoop(self)
	...
	if self:IsMythic() then
		if self.vb.phase == 2 then
			if self.vb.stupefyingGlareCount % 2 == 0 then
				direction = DBM_CORE_Translations.RIGHT--ie counter clockwise
			else
				direction = DBM_CORE_Translations.LEFT--ie Clockwise
			end
		else--Phase 3
			if self.vb.stupefyingGlareCount % 2 == 0 then
				direction = DBM_CORE_Translations.LEFT--ie counter clockwise
			else
				direction = DBM_CORE_Translations.RIGHT--ie Clockwise
			end
		end
	else--Not mythic
	...
	if self.Options.ArrowOnGlare then
		...
		if direction == DBM_CORE_Translations.LEFT then
			DBM.Arrow:ShowStatic(90, 10)
		elseif direction == DBM_CORE_Translations.RIGHT then
	...
	if timer then
		if self:IsMythic() then
			...
			if direction == DBM_CORE_Translations.RIGHT then
				direction = DBM_CORE_Translations.LEFT
			elseif direction == DBM_CORE_Translations.LEFT then
				direction = DBM_CORE_Translations.RIGHT
			end
		else
			--Right, Left, Left for LFR at least, assumed rest same since timers are
			--TODO, verify normal and heroic one day, or maybe users will at least report it if it's wrong
			if self.vb.stupefyingGlareCount == 1 or self.vb.stupefyingGlareCount == 2 then
				direction = DBM_CORE_Translations.LEFT--ie counter clockwise for next one
			end
		end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif spellId == 313184 or spellId == 319309 then
		...
		if spellId == 319309 then--Add deadly icon to this on mythic
			timerSynampticShock:UpdateInline(DBM_CORE_Translations.DEADLY_ICON)
		end
	elseif spellId == 316541 or spellId == 316542 then
		...
		if #ParanoiaTargets % 2 == 0 then
			...
			if playerIsInPair then--Only repeat yell on mythic and mythic+
				...
				if type(icon) == "number" then icon = DBM_CORE_Translations.AUTO_YELL_CUSTOM_POSITION:format(icon, "") end
