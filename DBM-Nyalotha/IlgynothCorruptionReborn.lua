...
--Stage 01: The Corruptor, Reborn
local timerEyeofNZothCD						= mod:NewCDTimer(17, 309961, nil, "Tank", nil, 5, nil, DBM_CORE_Translations.TANK_ICON, nil, 2, 3)--16.6-17.4 (0ld), new seems more stable 17
local timerTouchoftheCorruptorCD			= mod:NewCDCountTimer(64.4, 311401, nil, nil, nil, 3, nil, DBM_CORE_Translations.HEROIC_ICON, nil, 1, 4)--64.4-68
local timerCorruptorsGazeCD					= mod:NewCDCountTimer(32.2, 310319, 202046, nil, nil, 3)--32.8-34 Shorttext "Beam"
--Stage 02: The Organs of Corruption
local timerCursedBloodCD					= mod:NewNextCountTimer(18, 311159, nil, nil, nil, 3, nil, DBM_CORE_Translations.DEADLY_ICON)
local timerAbsorbingChargeCD				= mod:NewNextTimer(18.3, 318383, nil, nil, nil, 5, nil, DBM_CORE_Translations.TANK_ICON)
...
function mod:OnCombatStart(delay)
	...
	if self:IsHard() then
		...
		if self:IsMythic() then
			...
			timerCursedBloodCD:UpdateInline(DBM_CORE_Translations.MAGIC_ICON)
		end
	end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif spellId == 311159 or spellId == 314396 then--Non Mythic, Mythic
		...
		if self:IsMythic() then
			self.vb.bloodIcon = 1
			timerCursedBloodCD:UpdateInline(DBM_CORE_Translations.MAGIC_ICON, self.vb.bloodCount+1)
		end
	end
end
