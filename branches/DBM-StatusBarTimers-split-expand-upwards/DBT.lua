...
function barPrototype:Update(elapsed)
	...
	if isMoving == "move" and self.moveElapsed <= 0.5 then
		...
		local newY
		if obj.options.ExpandHugeUpwards then
	...
	elseif isMoving == "next" then
		...
		local newY
		if obj.options.ExpandUpwards and self.enlarged then
...
options.ExpandHugeUpwards.onChange = updateOrientation
...
function barPrototype:Enlarge()
	...
	local oldY = self.frame:GetTop()
	self.frame:ClearAllPoints()
	if self.owner.options.ExpandHugeUpwards then
