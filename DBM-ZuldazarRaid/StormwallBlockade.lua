...
local timerElecShroudCD					= mod:NewCDTimer(34, 287995, nil, nil, nil, 4, nil, DBM_CORE_Translations.DAMAGE_ICON..DBM_CORE_Translations.INTERRUPT_ICON)--34-41, maybe health based?
----Brother Joseph
mod:AddTimerLine(DBM:EJ_GetSectionInfo(19262))
local timerSeaStormCD					= mod:NewCDTimer(10.9, 284360, nil, nil, nil, 3)
local timerSeasTemptationCD				= mod:NewCDCountTimer(34, 284383, nil, nil, nil, 1, nil, DBM_CORE_Translations.DAMAGE_ICON)--Might be health based
local timerTidalShroudCD				= mod:NewCDTimer(36.5, 286558, nil, nil, nil, 4, nil, DBM_CORE_Translations.DAMAGE_ICON..DBM_CORE_Translations.INTERRUPT_ICON)--Probalby health based
--Stage Two: Laminaria
mod:AddTimerLine(DBM:EJ_GetSectionInfo(19258))
local timerCataTides					= mod:NewCastTimer(15, 288696, nil, nil, nil, 4, nil, DBM_CORE_Translations.INTERRUPT_ICON)
local timerSeaSwellCD					= mod:NewCDTimer(20.6, 285118, nil, nil, nil, 3, nil, DBM_CORE_Translations.DEADLY_ICON, nil, 1, 3)
...
local timerJoltingVolleyCD				= mod:NewCDCountTimer(43.6, 287169, nil, nil, nil, 2, nil, DBM_CORE_Translations.HEALER_ICON)
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif spellId == 285350 or spellId == 285426 then
		if args:IsPlayer() then
			...
			if expireTime then
				local remaining = expireTime-GetTime()
				specWarnStormsWail:Schedule(remaining-4.5, DBM_CORE_Translations.BACK)
