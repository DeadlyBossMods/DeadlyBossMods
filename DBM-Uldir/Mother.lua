...
local timerSanitizingStrikeCD			= mod:NewNextTimer(23.1, 267787, 191540, "Tank", nil, 5, nil, DBM_CORE_Translations.TANK_ICON, nil, 2, 3)--Short name "Strike"
local timerPurifyingFlameCD				= mod:NewNextTimer(20.1, 267795, nil, nil, nil, 3, nil, DBM_CORE_Translations.DEADLY_ICON, nil, 1, 3)
local timerWindTunnelCD					= mod:NewNextTimer(39.5, 267945, nil, nil, nil, 2)
local timerSurgicalBeamCD				= mod:NewCDSourceTimer(30, 269827, 143444, nil, nil, 3, nil, DBM_CORE_Translations.DEADLY_ICON, nil, 3, 3)--Shortname "Laser"
...
local function updateAllTimers(self, ICD)
	...
	if self.vb.phase >= 2 then
		if self:IsMythic() then
			if timerSurgicalBeamCD:GetRemaining(DBM_CORE_Translations.BOTH) < ICD then
				local elapsed, total = timerSurgicalBeamCD:GetTime(DBM_CORE_Translations.BOTH)
				local extend = ICD - (total-elapsed)
				DBM:Debug("timerSurgicalBeamCD BOTH extended by: "..extend, 2)
				timerSurgicalBeamCD:Stop()
				timerSurgicalBeamCD:Update(elapsed, total+extend, DBM_CORE_Translations.BOTH)
			end
		else
			if (self.vb.nextLaser == 1) and timerSurgicalBeamCD:GetRemaining(DBM_CORE_Translations.SIDE) < ICD then
				local elapsed, total = timerSurgicalBeamCD:GetTime(DBM_CORE_Translations.SIDE)
				local extend = ICD - (total-elapsed)
				DBM:Debug("timerSurgicalBeamCD SIDE extended by: "..extend, 2)
				timerSurgicalBeamCD:Stop()
				timerSurgicalBeamCD:Update(elapsed, total+extend, DBM_CORE_Translations.SIDE)
			end
			if (self.vb.nextLaser == 2) and timerSurgicalBeamCD:GetRemaining(DBM_CORE_Translations.TOP) < ICD then
				local elapsed, total = timerSurgicalBeamCD:GetTime(DBM_CORE_Translations.TOP)
				local extend = ICD - (total-elapsed)
				DBM:Debug("timerSurgicalBeamCD TOP extended by: "..extend, 2)
				timerSurgicalBeamCD:Stop()
				timerSurgicalBeamCD:Update(elapsed, total+extend, DBM_CORE_Translations.TOP)
			end
		end
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif spellId == 269827 or spellId == 277973 or spellId == 277961 or spellId == 277742 then
		if self:IsMythic() then--All the things
			specWarnSurgicalBeam:Show(DBM_CORE_Translations.BOTH)
			if self.vb.phase == 3 then
				timerSurgicalBeamCD:Start(20.5, DBM_CORE_Translations.BOTH)--20, but almost always delayed by ICD
			else
				timerSurgicalBeamCD:Start(50, DBM_CORE_Translations.BOTH)--50, but often delayed by ICD
			end
			updateAllTimers(self, 8.5)
		elseif self:IsEasy() then--Only side
			specWarnSurgicalBeam:Show(DBM_CORE_Translations.SIDE)
			timerSurgicalBeamCD:Start(30, DBM_CORE_Translations.SIDE)--30-31
			self.vb.nextLaser = 1
			updateAllTimers(self, 8.5)
		else--Heroic (alternating)
			if spellId == 277961 or spellId == 277742 or spellId == 269827 then--Top spellIds
				specWarnSurgicalBeam:Show(DBM_CORE_Translations.TOP)
				--Next Beam side
				timerSurgicalBeamCD:Start(11, DBM_CORE_Translations.SIDE)--Usually delayed, but yes it's 11
				self.vb.nextLaser = 1
				...
				self.vb.nextLaser = 2
				if self.vb.phase == 3 then
					timerSurgicalBeamCD:Start(24, DBM_CORE_Translations.TOP)
					...
					timerSurgicalBeamCD:Start(29, DBM_CORE_Translations.TOP)
				end
				updateAllTimers(self, 8.5)
			end
		end
	...
	elseif spellId == 269051 then--Begin Cast of Cleansing Purge
		...
		elseif cid == 137022 then
			...
			if not self.vb.bossInICD then
				timerSurgicalBeamCD:Start(5, DBM_CORE_Translations.SIDE)
			else
				timerSurgicalBeamCD:Start(10, DBM_CORE_Translations.SIDE)
			end
		elseif cid == 137023 then
			self.vb.phase = 3
			self.vb.nextLaser = 1
			timerSurgicalBeamCD:Stop()--Resets, kinda
			timerSurgicalBeamCD:Start(15, DBM_CORE_Translations.SIDE)--15 if delayed by nothing, but can be longer if flames ICD gets triggered
