...
----Arena Floor P2+
local specWarnGrowingCorruption			= mod:NewSpecialWarningCount(270447, nil, DBM_CORE_Translations.AUTO_SPEC_WARN_OPTIONS.stack:format(5, 270447), nil, 1, 2)
...
----Adds
local timerMassiveSmashCD				= mod:NewCDTimer(9.7, 267412, nil, "Tank", nil, 5, nil, DBM_CORE_Translations.TANK_ICON)
local timerDarkBargainCD				= mod:NewCDTimer(23.1, 267409, nil, nil, nil, 3, nil, DBM_CORE_Translations.DAMAGE_ICON)
...
local timerBurstingBoil					= mod:NewCastCountTimer(8, 277007, nil, nil, 2, 5, nil, DBM_CORE_Translations.MYTHIC_ICON)
local timerBurstingBoilCD				= mod:NewCDCountTimer(20.5, 277007, nil, nil, nil, 3, nil, DBM_CORE_Translations.MYTHIC_ICON)
----Horror
...
mod.vb.matrixSide = DBM_CORE_Translations.RIGHT
		...
		if mod.vb.phase < 3 then
			...
			if mod.vb.matrixActive then
				...
				if mod:IsMythic() then
					...
					local sideText = (mod.vb.matrixSide == DBM_CORE_Translations.LEFT) and DBM_CORE_Translations.RIGHT or DBM_CORE_Translations.LEFT
...
function mod:OnCombatStart(delay)
	...
	if not self:IsMythic() then
		self.vb.matrixSide = DBM_CORE_Translations.RIGHT
...
function mod:SPELL_CAST_START(args)
	...
	elseif (spellId == 263482 or spellId == 263503) then
		self.vb.matrixActive = false
		self.vb.matrixSide = DBM_CORE_Translations.RIGHT--Actually left, but this makes it so the way it's coded works
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg, npc, _, _, target)
	if msg:find("spell:263420") and self:AntiSpam(10, 7) then
		...
		if not self:IsMythic() then
			if self.vb.matrixSide == DBM_CORE_Translations.LEFT then
				self.vb.matrixSide = DBM_CORE_Translations.RIGHT
			else
				self.vb.matrixSide = DBM_CORE_Translations.LEFT
			end
