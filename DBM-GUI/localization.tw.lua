...
L.Area_ImportExportSpellRenames	= "匯入/匯出 法術重新命名"
L.ImportExportSpellRenamesInfo	= "匯入會覆寫您當前的法術重新命名設定，後果請自負。"
L.ButtonImportSpellRenames			= "匯入法術重新命名"
L.ButtonExportSpellRenames			= "匯出法術重新命名"
L.ExportSpellRenamesFailed			= "匯出法術重新命名字串失敗。資料可能無效/損毀，或來自不支援的格式。"
L.ImportSpellRenamesFailed			= "匯入法術重新命名字串失敗。資料可能無效/損毀，或來自不支援的格式。"
L.ImportSpellRenamesWrongType		= "匯入法術重新命名字串失敗。此匯出類型不是法術重新命名資料。"
L.ImportSpellRenamesUnsupportedVersion	= "匯入法術重新命名字串失敗。此匯出版本不受支援。"
L.SpellRenamesImported				= "法術重新命名已匯入。"
L.ImportErrorOn						= "缺少設置中的自定義聲音: %s"
L.ImportVoiceMissing				= "缺少語音包: %s"
L.LegacyProfileImportNotice		= "已匯入舊版設定檔。此格式已被淘汰，請使用新格式重新匯出。"
