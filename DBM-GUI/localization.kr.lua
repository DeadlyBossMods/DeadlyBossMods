...
L.BossModLoad_now 			= [[이 보스 모드를 불러오지 않았습니다. 
해당 인스턴스에 진입하면 불러올 것입니다.
직접 버튼을 클릭해서 모드를 불러올 수도 있습니다.]]
...
L.General 					= "일반 DBM 핵심 설정"
L.EnableMiniMapIcon			= "미니맵 버튼 표시"
L.UseSoundChannel			= "DBM 경보 효과음 출력 채널 선택"
L.UseMasterChannel			= "주 오디오"
...
L.ModelSoundOptions			= "모델 뷰어에서 효과음 설정"
...
L.CoreMessages				= "핵심 메시지 설정"
L.ShowPizzaMessage 			= "대화창에 전송받은 타이머 표시"
L.ShowAllVersions	 		= "버전 검사시 대화창에 모든 파티/공격대원의 보스 모드 버전을 표시합니다. (설정을 꺼도 구버전/신버전으로 요약 출력)"
...
L.Slider_BarScale 			= "바 스케일: %0.2f"
--Types
L.BarStartColorAdd			= "시작 색상 (쫄)"
L.BarEndColorAdd			= "종료 색상 (쫄)"
L.BarStartColorAOE			= "시작 색상 (광역뎀)"
L.BarEndColorAOE			= "종료 색상 (광역뎀)"
...
L.Bar7CustomInline			= "바 안쪽에 사용자 정의 '!' 아이콘 사용"
...
L.Bar_Decimal				= "남은시간 소수점: %d초 이하"
L.Bar_DBMOnly				= "아래 설정은 \"DBM\" 바 스타일에서만 적용됩니다."
L.Bar_EnlargeTime			= "일정 시간 미만 바 확대: %d초"
L.Bar_EnlargePercent		= "일정 비율 미만 바 확대: %0.1f%%"
L.BarSpark					= "바 끝 강조"
...
L.SWarnNameInNote			= "메모에 이름이 포함될 경우 SW5 설정 사용"
L.SpecialWarningIcon		= "특수 경고에 아이콘 사용"
L.SpecWarn_FlashFrame		= "특수 경고에 화면 점멸 효과 사용"
L.SpecWarn_FlashFrameRepeat	= "%d회 반복 (사용시)"
...
L.CountdownVoice			= "초세기 1순위 음성 설정"
L.CountdownVoice2			= "초세기 2순위 음성 설정"
L.CountdownVoice3			= "초세기 3순위 음성 설정"
L.VoicePackChoice			= "음성 경보에 쓸 음성팩 설정"
L.Area_CountdownOptions		= "초세기 설정"
L.ShowCountdownText			= "1순위 초세기 음성이 재생시 화면 중앙에 숫자 표시"
L.Area_VoicePackOptions		= "음성팩 설정 (써드파티 음성팩)"
L.SpecWarn_NoSoundsWVoice	= "음성 경보가 있는 특수 경고의 효과음 재생"
L.SWFNever					= "이 기능 사용 안함"
L.SWFDefaultOnly			= "특수 경고가 기본 효과음을 재생할 때만 (사용자 정의 효과음은 계속 재생 허용)"
L.SWFAll					= "특수 경고가 모든 효과음을 사용시"
...
-- Tab: Global Filter
L.Panel_SpamFilter			= "DBM 전역 기능 끄기"
L.Area_SpamFilter_Outgoing	= "DBM 전역 기능 끄기 설정"
L.SpamBlockNoShowAnnounce	= "알림 및 효과음 출력 안함"
L.SpamBlockNoSpecWarn		= "특수 경고 및 효과음 출력 안함"
L.SpamBlockNoShowTimers		= "모드 타이머 표시 안함 (보스 모드/도전모드/파티찾기/재생성)"
L.SpamBlockNoShowUTimers	= "사용자 전송 타이머 표시 안함 (사용자 정의/풀링/휴식)"
...
L.SpamBlockNoCountdowns		= "초세기 효과음 재생 안함"
L.SpamBlockNoYells			= "외침 알림 전송 안함"
L.SpamBlockNoNoteSync		= "메모 공유 수락 안함"

L.Area_Restore				= "DBM 복원 설정 (DBM이 보스 모드 종료시 이전 사용자 설정 상태로 돌아갈 지 여부)"
L.SpamBlockNoIconRestore	= "아이콘 상태를 저정하지 않고 전투 종료시 원래대로 복원 안함"
L.SpamBlockNoRangeRestore	= "모드가 '숨김' 명령을 내렸을 때 이전 상태로 거리 창 복원 안함"

-- Tab: Spam Filter
L.Area_SpamFilter			= "스팸 필터 설정"
L.DontShowFarWarnings		= "멀리 떨어진 곳의 이벤트에 대한 알림 및 바 표시 안함"
L.StripServerName			= "경고와 타이머에서 서버명 지우기"
L.SpamBlockBossWhispers		= "전투 중 &lt;DBM&gt; 귓속말 안봄"
...
L.FilterDispels				= "해제 주문이 대기시간이 도는 경우 해제 경고 보지 않기"
L.FilterSelfHud				= "HUD에서 자기 정보는 보지 않기 (거리 기반 HUD 기능은 제외)"

L.Area_PullTimer			= "풀링, 휴식, 전투, 사용자 정의 바 필터 설정"
L.DontShowPTNoID			= "같은 지역에 없는 사용자가 보낸 풀링 타이머 차단"
L.DontShowPT				= "풀링/휴식 타이머 보지 않기"
L.DontShowPTText			= "풀링/휴식 알림 텍스트 보지 않기"
L.DontPlayPTCountdown		= "풀링/휴식/전투/사용자 정의 초세기 음성"
L.DontShowPTCountdownText	= "풀링/휴식/전투/사용자 정의 초세기 텍스트"
L.PT_Threshold				= "휴식/전투/사용자 정의 타이머 초세기 텍스트 표시 안함: %d 이후"

L.Panel_HideBlizzard		= "블리자드 기능 끄기 및 숨김"
L.Area_HideBlizzard			= "블리자드 기능 끄기 및 숨김 설정"
L.HideBossEmoteFrame		= "보스 전투중 보스 감정표현 숨기기"
L.HideWatchFrame			= "추적중인 업적이 없을 경우 전투 중 퀘스트 추적 프레임을 숨깁니다. 도전 모드에서는 프레임이 숨겨지는 동안에 임시 메달 타이머가 생성됩니다."
L.HideGarrisonUpdates		= "보스 전투중 주둔지 팝업창 숨기기"
L.HideGuildChallengeUpdates	= "보스 전투중 길드 도전 과제 알림창 숨기기"
L.HideQuestTooltips			= "보스 전투중 툴팁의 퀘스트 목표 내역 숨기기"
L.HideTooltips				= "보스 전투 중 툴팁 완전히 숨기기"
L.DisableSFX				= "보스 전투중 소리 채널 끄기"
L.SpamBlockSayYell			= "대화창의 말풍선 알림 숨기기"
...
L.LFDEnhance				= "역할 확인 및 전장/공격대 찾기가 열릴 때 전투 준비 효과음 재생(주 오디오 또는 대화 채널) 및 와우 트레이 아이콘 점멸 (참고: 소리 채널이 꺼져있어도 작동하며, 일반적으로 평소보다 더 크게 들립니다)"
...
L.DualProfile				= "전문화 별로 다른 보스 모드를 사용합니다. (보스 모드 프로필 관리는 불러낸 보스 모드의 통계 화면에서 이루어집니다)"

L.Area_ModProfile			= "다른 캐릭터/전문화의 설정 복사/삭제"
L.ModAllReset				= "모든 모드의 설정 초기화"
L.ModAllStatReset			= "모든 모드의 통계 초기화"
