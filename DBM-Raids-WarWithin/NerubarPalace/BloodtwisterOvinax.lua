...
mod:RegisterEventsInCombat(
	"SPELL_CAST_START 442526 442432 446349 441362 443005 446700",
	"SPELL_CAST_SUCCESS 442430",
...
local timerExperimentalDosageCD					= mod:NewAITimer(49, 442526, nil, nil, nil, 3)
local timerIngestBlackBloodCD					= mod:NewAITimer(49, 442430, nil, nil, nil, 3)
local timerUnstableWebCD						= mod:NewAITimer(49, 446349, nil, nil, nil, 3, nil, DBM_COMMON_L.HEROIC_ICON..DBM_COMMON_L.MAGIC_ICON)
local timerVolatileConcoctionCD					= mod:NewAITimer(49, 441362, nil, nil, nil, 5, nil, DBM_COMMON_L.TANK_ICON)
...
--local timerPoisonBurstCD						= mod:NewCDNPTimer(49, 446700, nil, nil, nil, 4)
...
function mod:OnCombatStart(delay)
	...
	self.vb.webCount = 0
	self.vb.tankCount = 0
	timerExperimentalDosageCD:Start(1)
	timerIngestBlackBloodCD:Start(1)
	timerVolatileConcoctionCD:Start(1)
	if self:IsHard() then
		timerUnstableWebCD:Start(1)
	end
...
function mod:SPELL_CAST_START(args)
	...
	if spellId == 442526 then
		...
		timerExperimentalDosageCD:Start()
	...
	elseif spellId == 441362 then--Nature
		self.vb.tankCount = self.vb.tankCount + 1
		if self:IsTanking("player", "boss1", nil, true) then
			specWarnVolatileConcoction:Show()
			specWarnVolatileConcoction:Play("defensive")
		end
		timerVolatileConcoctionCD:Start()
	elseif spellId == 443005 then--Shadow
		self.vb.tankCount = self.vb.tankCount + 1
		if self:IsTanking("player", "boss1", nil, true) then
			specWarnVolatileConcoction:Show()
			specWarnVolatileConcoction:Play("defensive")
